...
# $2 - process VIS? True/False
# $3, etc. - arguments to measure.sh
# Run this script with the following command:
# submit_measure.sh <label> <process_vis> <args>
...
label=$1; shift
process_vis=${1,,}; shift  # this syntax converts to lowercase
args="$@"
measure_mask="measure.py $args --create-masks"
measure_iso="measure.py $args --measure-isophotes --isophotes-filter"
measure_phot="measure.py $args --measure-photometry --photometry-filter"
...
JOB_PHOT_Y_YJH=$(sbatch --parsable --job-name=phot_${label}_Y_YJH --dependency=afterok:$JOB_ISO_YJH:$JOB_PHOT_Y_Y $measure_phot Y --isophotes-filter YJH)
...
JOB_PHOT_J_YJH=$(sbatch --parsable --job-name=phot_${label}_J_YJH --dependency=afterok:$JOB_ISO_YJH:$JOB_PHOT_J_J $measure_phot J --isophotes-filter YJH)
...
if [[ "$process_vis" == "true" ]]; then
    ...
    JOB_PHOT_VIS_YJH=$(sbatch --parsable --job-name=phot_${label}_VIS_YJH --dependency=afterok:$JOB_ISO_YJH:$JOB_PHOT_VIS_VIS $measure_phot VIS --isophotes-filter YJH)
fi
