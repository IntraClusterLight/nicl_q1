...
JOB_MASK_YJH=$(sbatch --parsable --job-name=mask_${label}_YJH measure_test_1_mask.sh $1 YJH)
...
JOB_PHOT_Y_YJH=$(sbatch --parsable --job-name=phot_${label}_Y_YJH --dependency=afterok:$JOB_ISO_Y measure_test_3_photometry.sh $1 Y YJH $2)
...
JOB_PHOT_J_YJH=$(sbatch --parsable --job-name=phot_${label}_J_YJH --dependency=afterok:$JOB_ISO_J measure_test_3_photometry.sh $1 J YJH $2)
...
JOB_PHOT_H_YJH=$(sbatch --parsable --job-name=phot_${label}_H_YJH --dependency=afterok:$JOB_ISO_H measure_test_3_photometry.sh $1 H YJH $2)


JOB_MASK_VIS=$(sbatch --parsable --job-name=mask_${label}_VIS measure_test_1_mask.sh $1 VIS)
...
JOB_PHOT_VIS_YJH=$(sbatch --parsable --job-name=phot_${label}_VIS_YJH --dependency=afterok:$JOB_ISO_VIS measure_test_3_photometry.sh $1 VIS YJH $2)