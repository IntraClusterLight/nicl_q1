...
def main():
    ...
    credential = {'account_name': args.user, 'password': getPassword(args.password_env)}
    ...
    try:
        if args.preview:
            preview(credential, sql, sys.stdout, args.release_version)
        else:
            job = submitJob(credential, sql, args.out_format,
                            args.release_version, args.nomail,
                            args.skip_syntax_check)
...
def submitJob(credential, sql, out_format,
              ...
              nomail=True, skip_syntax_check=False):
    url = api_url + 'submit'
    catalog_job = {
        'sql'                     : sql,
...
def preview(credential, sql, out, release_version):
    url = api_url + 'preview'
    catalog_job = {
        'sql'             : sql,
        'release_version' : release_version,
