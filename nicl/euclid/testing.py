...
def create_test_images_with_shapes_icl_offset(
    ...
    icl_q=0.6,
    icl_theta=60,
    icl_offset=(0, 0),  # NEW: Offset in pixels (dx, dy)
):
    path = default_data_path("Q1_R1_clusters_test", "MCXCJ1754.6+6803")
    hdul = fits.open(path / "EUC_NIR_W-STK_H-MCXCJ1754.6+6803.fits")
    ...
    if cluster_redshift:
        ...
        bcg = galsim.Sersic(n=bcg_n, half_light_radius=bcg_re, flux=1)
        bcg = bcg.shear(q=bcg_q, beta=bcg_theta * galsim.degrees)
        
        icl_re = (physical_to_angular(189.5 * u.kpc, cluster_redshift) / pixscale).to_value(u.pix)
        icl = galsim.Sersic(n=icl_n, half_light_radius=icl_re, flux=1)
        icl = icl.shear(q=icl_q, beta=icl_theta * galsim.degrees)
    ...
    for band in ["H", "J", "Y", "VIS"]:
        ...
        if cluster_redshift:
            ...
            print(f"{band} total_flux: {bcg_flux + icl_flux}")

            image = galsim.ImageF(*shape)

            if icl_offset == (0, 0):
                # Original combined method
                cluster = bcg * bcg_flux + icl * icl_flux
                cluster.drawImage(image=image, method="no_pixel")
                combined = image.array
            else:
                # New -- individual shapes with offset
                bcg.drawImage(
                    image=image,
                    method="no_pixel",
                ...
                image.array *= bcg_flux
                bcg_array = image.array.copy()

                # Draw ICL with offset
                image.array[:] = 0
                icl.drawImage(
                    image=image,
                    method="no_pixel",
                    offset=galsim.PositionD(*icl_offset),
                    
                )
                image.array *= icl_flux
                icl_array = image.array.copy()

                combined = bcg_array + icl_array

            hdul["SCI"].data[:] = combined
            hdul.writeto(outpath / f"{label}_{band}_no_noise.fits", overwrite=True)

            # Add source noise
            source_noise = np.random.normal(
                scale=np.maximum(0, source_noise_level * combined), size=shape
            )
            hdul["SCI"].data[:] += source_noise
        ...
        if background_filenames is not None:
            ...
            bkg = bkg_hdul["SCI"].data
            rng = np.random.default_rng(background_seed)
            max_y = bkg.shape[0] - shape[0]
            max_x = bkg.shape[1] - shape[1]
            if max_y < 0 or max_x < 0:
                raise ValueError(
                    f"Background image size {bkg.shape} is too small for required shape {shape}"
                )
            start_y = rng.integers(0, max_y + 1)
            start_x = rng.integers(0, max_x + 1)
            bkg = bkg[start_y:start_y + shape[0], start_x:start_x + shape[1]]
            ...
            background_rms = (
                background_rms_level * rms[band] if background_rms_level else None
            )
            _, _, background = create_test_background(
                shape,
                rms[band],
