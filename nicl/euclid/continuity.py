...
# %% ../../nbs/euclid/continuity.ipynb #7a239924
import logging
...
# %% ../../nbs/euclid/continuity.ipynb #6db55f49
...
# %% ../../nbs/euclid/continuity.ipynb #65fe3248
def bkg_match_corr(
    ...
    The function identifies overlapping regions between images by shifting one by
    an integer to match another and computes the differences therein. A system of
    linear equations is constructed from the overlaps and solved using least
    squares to find the optimal additive (and optionally polynomial) corrections
    that minimize offsets in overlapping regions. The corrections are applied
    in-place to the image data.

    Parameters
    ----------
    paths : list of Path
        List of file paths to the images to be corrected. Each image should have a
        corresponding weight map with the suffix '.weight.fits'.
    corr_order : int, optional
        Order of the correction polynomial to apply. 0 for constant offset, 1 for
        first-order (linear), 2 for second-order (quadratic). Default is 0.
    ...
    Returns
    -------
    imgs_not_in_eqs : list of Path
        List of image paths that do not have any useful overlap with others and
        thus were not corrected.

    Notes
    -----
    - Source pixels are masked using an object mask generated by `fast_mask`.
    - Only corr_order=0 is currently implemented; higher orders will raise
      NotImplementedError.
    ...
    if n_imgs > n_imgs_in_eqs:
        ...
        logger.info("The following images have no useful overlap with others:")
        for idx in idx_imgs_not_in_eqs:
            imgs_not_in_eqs.append(paths[idx])
            logger.info(f"{paths[idx].name}")
