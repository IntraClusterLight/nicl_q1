...
# %% auto 0
__all__ = ['form_exp_pairs', 'do_resamp_images_overlap', 'bkg_match', 'slice_to_coords', 'can_wcs_shift_by_int',
           'get_overlap_slices', 'get_overlap_chip', 'compute_offset_between_chips', 'polygon_photometry',
           'form_dither_pairs', 'measure_offset_per_dither_pair', 'measure_offset_per_observation',
           'solve_for_correction_per_observation', 'apply_correction', 'correct_dither']
...
def form_exp_pairs(ids):
    """Form exposure pairs based on obs_id and dither_id."""
    pairs = []
    for i, (obs_id, dither_id) in enumerate(ids):
        for j in range(i + 1, len(ids)):
            obs_id2, dither_id2 = ids[j]
            if obs_id != obs_id2 or dither_id != dither_id2:
                pairs.append(((obs_id, dither_id), (obs_id2, dither_id2)))
    return pairs
...
def bkg_match(
    ...
    for i, path in enumerate(paths):
        ...
        other_paths = []
        for other in others:
            other_obs_id, other_dither_id = other
            # find the path for the other exposure
            for j, path2 in enumerate(paths[i + 1 :]):
                obs_id_, dither_id_ = ids[i + j + 1]
