...
# %% auto 0
__all__ = ['create_coarse_data', 'group_obs_ids', 'normalise_per_dither', 'prepare_group_data', 'construct_skyflats',
           'construct_skyflats_err', 'write_skyflats', 'read_skyflat', 'correct_for_zp', 'interpolate_skyflats',
           'create_skyflats', 'apply_skyflat']
...
def construct_skyflats(obs_id, group_data):
...
def construct_skyflats_err(obs_id, group_data, median):
...
def interpolate_skyflats(flat, data, method="nearest"):
    ...
    if isinstance(flat, xr.DataArray) or isinstance(flat, xr.Dataset):
        ...
        coarse_factor = np.ceil(np.array(data.shape) / np.array(flat.shape)).astype(int)
        ...
        for i in range(len(data.shape)):
            n_extra = data.shape[i] % coarse_factor[i]
            if n_extra != 0:
                extra_pix = coarse_factor[i] * (flat.shape[i] - 1) + n_extra / 2 - 0.5
...
def create_skyflats(
    ...
    if normalise == "zero":
        coarse_data = normalise_per_dither(coarse_data, restore_median_level=False)
    elif normalise == "median":
        coarse_data = normalise_per_dither(coarse_data, restore_median_level=True)
    flats = construct_skyflats(obs_id, coarse_data)
    if filter_size is not None: 
        flats = flats.rolling(x=filter_size, y=filter_size, min_periods=1, center=True).median()
    return flats

# %% ../../nbs/euclid/skyflat.ipynb 17
def apply_skyflat(data, skyflat, interpolation_method="nearest"):
    """Apply a coarse `skyflat` to some `data`."""
    skyflat_interp = interpolate_skyflats(skyflat, data, interpolation_method)
