...
# %% auto #0
__all__ = ['get_label_at_position', 'keep_segment_at_position', 'remove_segment_at_position', 'fft_binary_erosion',
           'fft_binary_dilation', 'smooth_image', 'create_bcg_mask', 'create_icl_mask', 'fast_mask',
           'dilated_object_mask', 'create_object_mask', 'create_faint_mask', 'plot_mask', 'write_mask']

# %% ../nbs/13_mask.ipynb #17b8d215-dad8-4793-b8d0-94ddfe1f85a4
import logging
...
from nicl.filter import sampled_mad_filter, sampled_median_filter
from nicl.utilities import get_img_centre_pixel

# %% ../nbs/13_mask.ipynb #3d2f0723
...
# %% ../nbs/13_mask.ipynb #1a74087e
def _convolve_and_threshold(x, structure, iterations, threshold):
    for i in range(iterations):
        x = fftconvolve(x, structure, "same") > threshold
    return x


def fft_binary_erosion(x, structure, iterations=1):
    threshold = np.count_nonzero(structure) - 0.5
    return _convolve_and_threshold(x, structure, iterations, threshold)


def fft_binary_dilation(x, structure, iterations=1):
    threshold = 0.5
    return _convolve_and_threshold(x, structure, iterations, threshold)

# %% ../nbs/13_mask.ipynb #e5353335
...
# %% ../nbs/13_mask.ipynb #290601e7-d774-4709-8ed2-f96960876628
def create_bcg_mask(
...
# %% ../nbs/13_mask.ipynb #525d7e8d-1f98-4808-a460-79ef4aef0f0f
def fast_mask(
    image: np.ndarray,  # input image data to mask
    *,  # the following parameters must be provided as keyword arguments if required
...
# %% ../nbs/13_mask.ipynb #b1a375aa-a344-4b18-ab71-6d046bba042a
...
# %% ../nbs/13_mask.ipynb #5cd3e54e-2e4e-4ebc-b94a-19f5937f5743
...
# %% ../nbs/13_mask.ipynb #e4da3f57-67f3-440d-ad8b-8a80870f106e
def plot_mask(
    img,  # image to plot
    mask,  # mask to plot
    *,  # the following parameters must be provided as keyword arguments if required
...
# %% ../nbs/13_mask.ipynb #f1676eec-9e66-4a55-9e4c-d6eccafaa8d0
def write_mask(
